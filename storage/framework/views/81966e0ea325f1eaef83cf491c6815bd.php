<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="glass p-6 rounded-2xl border border-slate-200/50">
            ...
            <div class="text-3xl font-bold text-slate-900 mt-1"><?php echo e(\App\Models\Vaccine::count()); ?></div>
        </div>
        <div class="glass p-6 rounded-2xl border border-slate-200/50">
            <div class="text-xs font-semibold text-slate-500 uppercase tracking-widest">Activas</div>
            <div class="text-3xl font-bold text-emerald-600 mt-1"><?php echo e(\App\Models\Vaccine::where('status', 'ACTIVE')->count()); ?></div>
        </div>
        <div class="glass p-6 rounded-2xl border border-slate-200/50">
            <div class="text-xs font-semibold text-slate-500 uppercase tracking-widest">En Cuarentena</div>
            <div class="text-3xl font-bold text-amber-500 mt-1"><?php echo e(\App\Models\Vaccine::where('status', 'QUARANTINE')->count()); ?></div>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $vaccines; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $vaccine): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50/80 transition-colors group">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-slate-100 text-slate-600 border border-slate-200">
                                    <?php echo e($vaccine->code); ?>

                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm font-semibold text-slate-900"><?php echo e($vaccine->name); ?></div>
                                <div class="text-xs text-slate-500"><?php echo e($vaccine->presentation); ?> • <?php echo e($vaccine->dose_per_unit); ?> dosis/u</div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-sm text-slate-600"><?php echo e($vaccine->manufacturer ?? 'No especificado'); ?></span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="text-sm font-bold <?php echo e($vaccine->active_stock > 0 ? 'text-emerald-600' : 'text-slate-400'); ?>">
                                    <?php echo e($vaccine->active_stock ?? 0); ?>

                                </span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center space-x-2">
                                    <span class="text-sm font-bold <?php echo e($vaccine->quarantine_stock > 0 ? 'text-amber-600' : 'text-slate-400'); ?>">
                                        <?php echo e($vaccine->quarantine_stock ?? 0); ?>

                                    </span>
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($vaccine->quarantine_stock > 0): ?>
                                        <button 
                                            wire:click.stop="$dispatch('openAdjustmentModal', { vaccineId: <?php echo e($vaccine->id); ?>, type: 'QUARANTINE_RELEASE' })"
                            ...
                            <td class="px-6 py-4 text-center">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($vaccine->status === 'QUARANTINE'): ?>
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-100 text-amber-800">
                                        Cuarentena
                                    </span>
                                <?php else: ?>
                                    <button 
                                        wire:click="toggleStatus(<?php echo e($vaccine->id); ?>)"
                                        class="relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none <?php echo e($vaccine->status === 'ACTIVE' ? 'bg-emerald-500' : 'bg-slate-300'); ?>"
                                    >
                                        <span class="pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200 <?php echo e($vaccine->status === 'ACTIVE' ? 'translate-x-5' : 'translate-x-0'); ?>"></span>
                                    </button>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                                <div class="flex items-center space-x-3">
                                    <button 
                                        wire:click="openModal(<?php echo e($vaccine->id); ?>)"
                                    ...
                                    <button 
                                        wire:click.stop="$dispatch('openAdjustmentModal', { vaccineId: <?php echo e($vaccine->id); ?>, type: 'QUARANTINE_MOVE' })"
                                        <?php if($vaccine->active_stock <= 0): ?> disabled <?php endif; ?>
                                        class="p-2 text-slate-400 rounded-lg transition-all <?php echo e($vaccine->active_stock > 0 ? 'hover:text-amber-600 hover:bg-amber-50' : 'opacity-30 cursor-not-allowed'); ?>"
                                        title="<?php echo e($vaccine->active_stock > 0 ? 'Mover stock a Cuarentena' : 'No hay stock disponible para mover'); ?>"
                                    >
            ...
            >
                <div class="px-8 py-6">
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="text-xl font-bold text-slate-900"><?php echo e($selectedVaccineId ? 'Editar Vacuna' : 'Nueva Vacuna'); ?></h3>
                    ...
                    <form wire:submit.prevent="save" class="space-y-5">
                        <div class="grid grid-cols-2 gap-4">
                            <div class="col-span-2">
                                ...
                                <input wire:model="name" type="text" class="block w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-premium-500 focus:bg-white transition-all text-sm <?php $__errorArgs = ['name'];
...
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <span class="text-xs text-red-500 italic mt-1"><?php echo e($message); ?></span> <?php unset($message);
if (isset($__messageOriginal)) { $message = $__messageOriginal; }
endif;
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            ...
                            <div>
                                ...
                                <input wire:model="code" type="text" class="block w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-premium-500 focus:bg-white transition-all text-sm <?php $__errorArgs = ['code'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-500 <?php unset($message);
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            ...
                            <div>
                                ...
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__errorArgs = ['presentation'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <span class="text-xs text-red-500 italic mt-1"><?php echo e($message); ?></span> <?php unset($message);
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            ...
                            <div>
                                ...
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__errorArgs = ['initial_batch'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <span class="text-xs text-red-500 italic mt-1"><?php echo e($message); ?></span> <?php unset($message);
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            ...
                            <div>
                                ...
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__errorArgs = ['dose_per_unit'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <span class="text-xs text-red-500 italic mt-1"><?php echo e($message); ?></span> <?php unset($message);
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            ...
                            <div>
                                ...
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__errorArgs = ['optimal_stock'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <span class="text-xs text-red-500 italic mt-1"><?php echo e($message); ?></span> <?php unset($message);
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        ...
                        <div class="pt-6 flex justify-end space-x-3">
                            ...
                            <button type="submit" class="px-8 py-2.5 bg-premium-600 hover:bg-premium-700 text-white text-sm font-bold rounded-xl transition-all shadow-lg shadow-premium-600/20">
                                <?php echo e($selectedVaccineId ? 'Guardar Cambios' : 'Crear Vacuna'); ?>
...
<?php /**PATH D:\laragon\www\vax-control\resources\views/livewire/vaccine-manager.blade.php ENDPATH**/ ?>