    ...
    <div class="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-slate-100">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $locations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $location): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-slate-50 transition-all">
                        <td class="px-8 py-5">
                            <div class="text-sm font-bold text-slate-900"><?php echo e($location->name); ?></div>
                            <div class="text-xs text-slate-400"><?php echo e($location->description ?? 'Sin descripción'); ?></div>
                        </td>
                        <td class="px-8 py-5">
                        ...
                        <td class="px-8 py-5">
                            <button wire:click="toggleStatus(<?php echo e($location->id); ?>)" class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none <?php echo e($location->is_active ? 'bg-emerald-500' : 'bg-slate-200'); ?>">
                                <span class="pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out <?php echo e($location->is_active ? 'translate-x-5' : 'translate-x-0'); ?>"></span>
                            </button>
                        </td>
                        <td class="px-8 py-5 text-right">
                            <button wire:click="edit(<?php echo e($location->id); ?>)" class="text-premium-600 hover:text-premium-800 font-bold text-sm">Editar</button>
                        </td>
                    </tr>
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($showModal): ?>
        ...
        >
            ...
            <div class="inline-block align-bottom bg-white rounded-[2rem] text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full p-8">
                <h3 class="text-xl font-bold text-slate-900 mb-6"><?php echo e($selectedId ? 'Editar Dependencia' : 'Nueva Dependencia'); ?></h3>
...
$message = $__bag->first($__errorArgs[0]); ?> <span class="text-xs text-rose-500 font-bold"><?php echo e($message); ?></span> <?php unset($message);
...
<?php /**PATH D:\laragon\www\vax-control\resources\views/livewire/location-manager.blade.php ENDPATH**/ ?>