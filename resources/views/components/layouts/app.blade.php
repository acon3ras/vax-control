...
<head>
    ...
    <title>{{ $title ?? 'Control de Vacunas' }} - Sistema de Gestión</title>
    ...
    <div class="flex h-screen overflow-hidden" x-data="{ sidebarOpen: false }">
        ...
        >
            <div class="flex items-center space-x-3 px-6 h-20 border-b border-white/5">
                <img src="{{ asset('images/logo.png') }}" alt="Logo" class="w-12 h-12 object-contain hover:scale-105 transition-transform">
            ...
            <nav class="mt-6 px-4 space-y-1 overflow-y-auto h-[calc(100vh-5rem)]">
                <a href="{{ route('dashboard') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-xl {{ request()->routeIs('dashboard') ? 'bg-premium-500/10 text-premium-400 border border-premium-500/20' : 'text-slate-400 hover:text-white hover:bg-white/5 border border-transparent transition-all' }}">
                ...
                <a href="{{ route('activity') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-xl {{ request()->routeIs('activity') ? 'bg-premium-500/10 text-premium-400 border border-premium-500/20' : 'text-slate-400 hover:text-white hover:bg-white/5 border border-transparent transition-all' }}">
                ...
                <a href="{{ route('vaccines.index') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-xl {{ request()->routeIs('vaccines.index') ? 'bg-premium-500/10 text-premium-400 border border-premium-500/20' : 'text-slate-400 hover:text-white hover:bg-white/5 border border-transparent transition-all' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" /></svg>
                    Catálogo de Vacunas
                </a>

                <a href="{{ route('locations.index') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-xl {{ request()->routeIs('locations.index') ? 'bg-premium-500/10 text-premium-400 border border-premium-500/20' : 'text-slate-400 hover:text-white hover:bg-white/5 border border-transparent transition-all' }}">
                ...
                @auth
                    ...
                    <a href="{{ route('users.index') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-xl {{ request()->routeIs('users.index') ? 'bg-premium-500/10 text-premium-400 border border-premium-500/20' : 'text-slate-400 hover:text-white hover:bg-white/5 border border-transparent transition-all' }}">
                    ...
                    <a href="{{ route('audit-logs') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-xl {{ request()->routeIs('audit-logs') ? 'bg-premium-500/10 text-premium-400 border border-premium-500/20' : 'text-slate-400 hover:text-white hover:bg-white/5 border border-transparent transition-all' }}">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                        Bitácora (Auditoría)
                    </a>

                    <a href="{{ route('system.settings') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-xl {{ request()->routeIs('system.settings') ? 'bg-premium-500/10 text-premium-400 border border-premium-500/20' : 'text-slate-400 hover:text-white hover:bg-white/5 border border-transparent transition-all' }}">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
                ...
                <div class="relative ml-auto" x-data="{ open: false }">
                    <button @click="open = !open" class="flex items-center space-x-3 hover:bg-slate-100 rounded-xl p-2 transition-colors focus:outline-none">
                        <div class="text-right hidden sm:block">
                            ...
                            <div class="text-[10px] text-slate-500">{{ auth()->user()->roles->first()->label ?? ucfirst(auth()->user()->roles->first()->name ?? 'Sin Rol') }}</div>
                        </div>
                        <div class="w-10 h-10 rounded-full bg-slate-200 border-2 border-white flex items-center justify-center overflow-hidden shadow-sm ring-2 ring-transparent group-hover:ring-premium-500/20 transition-all">
                            <span class="text-slate-400 text-sm font-bold">{{ substr(auth()->user()->name, 0, 1) }}</span>
                        </div>
                    ...
                    <div x-show="open" 
                        ...
                        <div class="py-1">
                            <a href="{{ route('profile') }}" class="group flex items-center px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 hover:text-emerald-600">
                            ...
                            <a href="{{ route('access-logs') }}" class="group flex items-center px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 hover:text-purple-600">
                            ...
                            <a href="{{ route('changelog') }}" class="group flex items-center px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 hover:text-blue-600">
            ...
            <!-- Global Modals -->
            <livewire:inventory-adjuster />
        </div>
    </div>

    <script src="{{ asset('vendor/livewire/livewire.js') }}" data-csrf="{{ csrf_token() }}" data-update-uri="{{ asset('/livewire/update') }}" data-navigate-once="true"></script>
    <script>
