                    ...
                    @forelse ($movements as $movement)
                        <tr class="hover:bg-slate-50/80 transition-all">
                            <td class="px-8 py-5 whitespace-nowrap">
                                <div class="text-sm font-bold text-slate-900">{{ $movement->created_at->format('d/m/Y') }}</div>
                                <div class="text-xs text-slate-400">{{ $movement->created_at->format('H:i') }} hrs</div>
                            </td>
                            <td class="px-8 py-5">
                            ...
                            <td class="px-8 py-5">
                                ...
                                @endphp
                                <span class="inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-bold border uppercase {{ $typeClasses }}">
                                    {{ $typeLabel }}
                                </span>
                            </td>
                            <td class="px-8 py-5">
                                @foreach($movement->items as $item)
                                    <div class="mb-1">
                                        <div class="text-sm font-bold text-slate-900">{{ $item->vaccine->name }}</div>
                                        @php
                                        ...
                                        @endphp
                                        <div class="text-xs {{ $quantityColor }} font-bold">{{ $sign }}{{ $item->quantity }} dosis</div>
                                    </div>
                                @endforeach
                            </td>
                            <td class="px-8 py-5 text-center">
                                @if($movement->evidence_path)
                                    <a href="{{ asset('storage/' . $movement->evidence_path) }}" target="_blank" class="inline-flex items-center p-2 rounded-xl bg-premium-50 text-premium-600 hover:bg-premium-100 transition-all shadow-sm" title="Ver archivo adjunto (Evidencia)">
                            ...
                            <td class="px-8 py-5 text-sm text-slate-500 whitespace-normal break-words">
                                {{ $movement->notes ?: '-' }}
                                @if($movement->reason)
                                    <span class="block text-xs text-rose-500 font-bold mt-1">Causa: {{ $movement->reason }}</span>
                                @endif
                                @if($movement->destinationLocation)
                                    <span class="block text-xs text-indigo-500 font-bold mt-1">Destino: {{ $movement->destinationLocation->name }}</span>
                                @endif
                            </td>
                        </tr>
                    @empty
